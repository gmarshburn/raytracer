...
### Design Choices
In order to calculate intersections with different types of shapes, I created four different shape structs 
that each have their own method to trace the ray into the scene. Each shape uses its own implicit formula 
in its trace ray method to calculate the appropriate t value. If there is no intersection with the current
shape given the current ray, the t variable is set to a default value of FLT_MAX. Additionally, once a t
value has been calculated, each shape's trace ray method returns an IntersectInfo struct that contains the
corresponding shape's t value, ctm, normal in object space, and material for lighting. The IntersectInfo
struct corresponding to the shape with the smallest t in the scene for the current pixel is then used for
later lighting. Finally, I implemented a Camera class to model the camera in the 3D scene to easily store
and access values like the camera's view matrix and height angle.
